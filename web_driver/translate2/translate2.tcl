...
proc fsm_if {tag slash param text} {
    ...
    Fsm::Run translate2_fsm tmpdata
}
...
proc doit {symbol url_template p_data} {
    ...
    if {[catch {htmlparse::parse -cmd translate2::fsm_if $data} rc]} {
    	Fsm::Init_Fsm translate2_fsm
    	Fsm::Set_State translate2_fsm FIND_TITLE
        set web_data(urlerror) "$symbol FAIL htmlparse error"
	set web_data(htmlparse_error) $rc
	return
    }
    if {[Fsm::Is_In_Service translate2_fsm] == 1} {
        array set tmpdata {}
        translate2_fsm::Dump_Translate tmpdata
	array set web_data [array get tmpdata]
    	unset tmpdata
    } else {
        set web_data(urlerror) "$symbol FAILfsm [Fsm::Get_Error translate2_fsm]"
    	Fsm::Init_Fsm translate2_fsm
    	Fsm::Set_State translate2_fsm FIND_TITLE
    }
    Fsm::Init_Fsm translate2_fsm
